...
def bot_file_download(token, file_path):
    ...
    try:
        ...
        possible_paths = [
            f'/var/lib/telegram-bot-api/{token}/{file_path}',
            f'/var/lib/telegram-bot-api/{file_path}',
            f'/{file_path}' if file_path.startswith('var/') else None,
        ]
        ...
        return jsonify({
            ...
            'tried_paths': [p for p in possible_paths if p]
        }), 404
